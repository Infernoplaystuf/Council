...
class CouncilConsole(tk.Tk):
    ...
    def _build_agents_tab(self):
        ...
        self.agent_log = self._make_text(top, wrap="word", height=14, state="disabled")
        ...
        self.agent_log.pack(fill="both", expand=True)
    ...
    def _send(self):
        ...
        def worker():
            try:
                ...
                # ── RAG context for Writer ─────────────────────────
                rag_context = ""
                if use_rag and self.rag:
                    try:
                        self.rag.auto_index_if_stale()
                        rag_context = self.rag.search_for_context(user_text, max_tokens_approx=1500)
                ...
                # ── Build agents dict ──────────────────────────────
                if use_tp_agent and self.techpriest_agent:
                ...
                if use_in_agent and self.intern_agent:
                ...
                # Inject RAG context into Writer's extra context
                writer_model = self.writer
                if rag_context:
                    writer_model._rag_context = rag_context   # picked up in respond()
                ...
                # Patch Writer to include RAG context
                if rag_context:
                    original_writer_respond = self.writer.respond
                    def _writer_with_rag(prompt, **kwargs):
                        kwargs["extra_context"] = (kwargs.get("extra_context", "") + "\n\n" + rag_context).strip()
                        return original_writer_respond(prompt, **kwargs)
                    self.writer.respond = _writer_with_rag
                ...
                # Restore original writer.respond if patched
                if rag_context:
                    self.writer.respond = original_writer_respond
