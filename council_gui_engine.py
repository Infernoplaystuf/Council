...
# ============================================================
# The council, and the two stores it writes through
# ============================================================
# Moved to council_core so the Qt front end runs the same deliberation. All of
# it was already outside CouncilConsole and touched no widget — see that
# module's header for what that means and for the second, smaller
# implementation in agent_core.py that is NOT what runs.
#
# The private names stay: 55 sites in this module use them.
from council_core.deliberation import (          # noqa: E402
    AgentContext,
    AgentEvent,
    DeliberationOrchestrator,
    ModelAgent,
    ToolFn,
    _detect_user_question,
    _extract_tool_calls,
    _looks_like_two_questions,
    _peasant_quality_score,
    _strip_code_blocks,
    peasant_cross_exam,
)
from council_core.managers import (               # noqa: E402
...
class CouncilConsole(tk.Tk):
    ...
    def _build_gui_designer_tab(self):
        ...
        # camera app needs its SDK's env, not the Council's own Python.
        self._gd_runwith = _grw.RunWithBox(bar, get_dir=self._gd_dir,
                                           log=lambda t: self._gd_log(t))
        self._gd_runwith.pack(side="left", padx=(12, 0))
    ...
    def _gd_wire_window(self) -> None:
        ...
        def _apply(values):
            for k, v in values.items():
                if k == "requires":     # project-level, edited in this panel
                    proj.requires = _gpol.parse_requires(v)
                elif hasattr(proj.window, k):
                    setattr(proj.window, k, v)
            for e in _gpol.check_requires(proj.requires):
                self._gd_log(f"! {e}")
            ...
            self._gd_log(f"window: bg={proj.window.bg or '(none)'} "
                         ...
                         f"requires={', '.join(proj.requires) or '(none)'}")
        self.gui_canvas.attach_window(proj.window, _apply,
                                      requires=getattr(proj, "requires", []))
    ...
    def _gd_generate(self):
        ...
        def _work():
            ...
            try:
                ...
                # The project's declared `requires` widen its allowlist — a
                # camera app's SDK — and nothing else does. Run applies the
                # same gate, so this message is now a promise, not a warning.
                pol_ok, pol_errs = _gpol.validate_dir(
                    pdir, man.mode, spec.requires)
                out.append("policy: OK" if pol_ok
                           else "policy REFUSED — Run will not start it until "
                                "this is fixed:")
    ...
    def _gd_run(self):
        ...
        try:
            man = _gp.load_manifest(pdir)
            reqs = _gp.open_project(self._gd_project, vault_dir=VAULT_DIR).requires
        except Exception as exc:
            self._gd_log(f"could not start preview: {exc}")
            return
        # Policy gate (ENFORCED here now — it used to say "the app will not
        # run it" while Run launched it anyway), interpreter, self-check,
        # launch. Off the Tk thread; see gui_runner.run_checked.
        _grun.run_checked(pdir, python_spec=man.python, mode=man.mode,
                          requires=reqs, log=self._gd_log,
                          call_soon=lambda fn: self.after(0, fn))
